...
const ProfileHeader = () => (
  <div className="flex flex-col items-center mt-8">
    ...
    <p className="text-gray-600 mt-2">I focus on quality not the quantity of my fruits</p>
  </div>
);
...
const DetailsSection = () => (
  <div className="mt-8 px-4">
    <div className="flex justify-between items-center mb-2">
      <span>Area:</span>
      <button className="text-gray-600"><MapPin size={16} /></button>
    </div>
    <div className="flex justify-between items-center mb-2">
    ...
    <div className="grid grid-cols-3 gap-4">
      {[1, 2, 3, 4, 5, 6].map((item, index) => (
        index === 5 ? (
          ...
          <div key={item} className="relative rounded-lg overflow-hidden aspect-video">
            <img 
...
const TestimonialCard = ({ name, location }) => (
  <div className="bg-emerald-500 p-6 rounded-lg text-white">
    <div className="flex flex-col items-center">
      <img src="/api/placeholder/48/48" alt={name} className="w-12 h-12 rounded-full" />
      <h3 className="mt-2 font-semibold">{name}</h3>
      <p className="text-sm">{location}</p>
    </div>
    <p className="text-center mt-4">
      The rule of 25 says you need to sav get this number, first annual expenses.
    </p>
